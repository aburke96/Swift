...
class COSC4360: UIViewController {
    ...
    */
    ...
    @IBOutlet weak var sliderParticipation: UISlider!
    
    @IBAction func sliderParticipationAction(_ sender: UISlider) {
        let currentValue = Int(sender.value)
        lblAttendance.text = "\(currentValue)%"
        updateGrade()
    }
    ...
    func updateGrade() -> Void {
        ...
        let participationGrade = sliderParticipation.value
        
        let finalGrade = (hwGrade) + (projectGrade) + (attendanceGrade) + (participationGrade)
